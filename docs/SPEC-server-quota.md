...
`src/lib/accountBook.ts` creates a Firebase Auth user and writes `users/{uid}` in the same signup. The local store only caches the current uid. **Kayıt ol** starts that profile at 10 messages. **Giriş yap** restores the existing profile and does not mint quota or delete other users. The same email returns `auth/email-already-in-use`. Sending requires `accountId` (the Firebase uid).
