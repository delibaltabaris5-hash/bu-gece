...
The locked auth screen stacks **Kayıt ol** over **Gmail ile devam et**. The same Gmail control is on the email register and email login panels. It calls `Google.useAuthRequest` when the platform client id is set: `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID` or `EXPO_PUBLIC_GOOGLE_CLIENT_ID` on web, `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` on iOS, `EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID` on Android. Expo does not inline a bare `GOOGLE_CLIENT_ID`. A new Google email is stored at 10. An email that already exists, including a password account, keeps its stored remaining count. With no client id the button stays visible and names the missing variable. This repo does not ship a live Google Cloud client (see `.env.example`).
